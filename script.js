...
// 1. Функция для открытия/закрытия окна профиля
...
// 2. Загрузка данных из Supabase в поля и шапку
async function loadProfileData() {
  ...
  if (user) {
    ...
    // Отображаем имя в шапке (найди свой элемент LUILUI... и дай ему этот ID)
    const nameDisplay = document.getElementById("user-name-display");
    if (nameDisplay) {
      nameDisplay.innerText = metadata.first_name || user.email.split("@")[0];
    }

    // Заполняем поля в модальном окне профиля
    document.getElementById("edit-first-name").value =
      metadata.first_name || "";
    document.getElementById("edit-last-name").value = metadata.last_name || "";
    document.getElementById("edit-location").value = metadata.location || "";
    document.getElementById("edit-github").value = metadata.github || "";
  }
}
...
async function updateProfile() {
  ...
  if (error) {
    ...
    loadProfileData(); // Обновляем имя в шапке сразу
    toggleProfileModal(); // Закрываем окно
  }
}

// Вызываем загрузку данных при старте страницы, чтобы имя появилось сразу
...
function switchAuthMode() {
  ...
  currentRegStep = 1; // Сбрасываем шаг на первый
  ...
  const step2 = document.getElementById("reg-fields-step-2");
  const regConfirm = document.getElementById("reg-confirm-wrapper");
  ...
  if (isSignUpMode) {
    ...
    mainBtn.innerText = "Продолжить"; // Сначала "Продолжить", а не "Создать"
    ...
    regConfirm.classList.add("hidden");
...
async function handleAuthSubmit() {
  ...
  if (isSignUpMode) {
    ...
    if (currentRegStep === 1) {
      ...
      // ПЕРЕХОД НА ШАГ 2
      document.getElementById("auth-step-1").classList.add("hidden"); // Скрываем Email/Пароль
      document.getElementById("reg-fields-step-2").classList.remove("hidden"); // Показываем профиль

      document.getElementById("auth-title").innerText = "Профиль";
      document.getElementById("auth-main-btn").innerText = "Завершить";
      document.getElementById("auth-toggle-text").classList.add("hidden"); // Скрываем переключалку

      currentRegStep = 2;
      return; // Прерываем функцию, чтобы не отправилось в Supabase раньше времени
    }

    // ШАГ 2: ОТПРАВКА В SUPABASE (срабатывает, когда currentRegStep === 2)
    const userMetadata = {
      first_name: document.getElementById("reg-first-name")?.value || "",
      last_name: document.getElementById("reg-last-name")?.value || "",
      birthday: document.getElementById("reg-birthday")?.value || "",
    ...
    if (error) {
      ...
      // Скрываем всё лишнее и показываем OTP
      document.getElementById("reg-fields-step-2").classList.add("hidden");
      const otpGroup = document.getElementById("otp-group");
